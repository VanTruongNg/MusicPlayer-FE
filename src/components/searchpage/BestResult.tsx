...
const ImageContainer = styled.div`
  ...
  width: 92px;
  ${({ $isRound }) =>
    $isRound ? "border-radius: 50%;" : "border-radius: 4px;"}
  overflow: hidden;
  position: relative;
`;
...
interface BestResultProps {
  ...
  artist: string[] | null; // Thêm artist
  fileUrl: string | null; // Thêm fileUrl
  onPlay?: () => void; // Thêm callback onPlay
}

export default function BestResult({ title, category, cover_url }) {
  return (
    <Container>
      <TopTitle>Best Result</TopTitle>
      <CardContainer>
